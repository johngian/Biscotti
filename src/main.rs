...
fn get_combinations(input_matrix: InputMatrix) -> Vec<Vec<EnvVar>>{
    ...
    for (key, values) in input_matrix.iter() {
        ...
        for value in values.iter() {
            env_vars.push(EnvVar{name: key.to_string(), value: value.to_string()});
        }
...
fn run_commands(combinations: Vec<Vec<EnvVar>>, command: String) {
    ...
    for combination in combinations {
        let tuples: Vec<(String, String)> = combination.into_iter().map(|x| (x.name, x.value)).collect();
...
fn main() -> Result<(), String> {
    let args = get_args();
    let matrix_str = args.value_of("matrix").expect("No input given for `matrix`.");
    let command = args.value_of("command").expect("No input given for `command`.");
